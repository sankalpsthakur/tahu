...
######################################################################
def publishBirth(metricValue, timestamp):
    publishNodeBirth(metricValue, timestamp)
    publishDeviceBirth(metricValue, timestamp)
######################################################################

######################################################################
# Publish the NBIRTH certificate
######################################################################
def publishNodeBirth(metricValue, timestamp):
    ...
    addMetric(payload, "Node Control/Next Server", None, MetricDataType.Boolean, False, timestamp)
    addMetric(payload, "Node Control/Rebirth", None, MetricDataType.Boolean, False, timestamp)
    addMetric(payload, "Node Control/Reboot", None, MetricDataType.Boolean, False, timestamp)

    # Add some regular node metrics
    addMetric(payload, "Node Metric0", None, MetricDataType.Int32, metricValue, timestamp)
...
######################################################################
def publishDeviceBirth(metricValue, timestamp):
    ...
    addMetric(payload, "Device Metric0", None, MetricDataType.Int32, metricValue, timestamp)
...
######################################################################
def publishData(metricValue, timestamp):
    # Periodically publish some new data
    nPayload = sparkplug_b_pb2.Payload()
    nPayload.timestamp = timestamp
    nPayload.seq = getSeqNum()
    addMetric(nPayload, "Node Metric0", None, MetricDataType.Int32, metricValue, timestamp)
    dPayload = sparkplug.getDdataPayload()
    addMetric(dPayload, "Device Metric0", None, MetricDataType.Int32, metricValue, timestamp)

    byteArray = bytearray(nPayload.SerializeToString())
    client.publish("spBv1.0/" + myGroupId + "/NDATA/" + myNodeName, byteArray, 0, False)
    byteArray = bytearray(dPayload.SerializeToString())
    client.publish("spBv1.0/" + myGroupId + "/DDATA/" + myNodeName + "/" + myDeviceName, byteArray, 0, False)
######################################################################
...
# Get the BIRTH value and timestamp
metricValue = 0
timestamp = int(round(time.time() * 1000))

# Publish the birth certificates
publishBirth(metricValue, timestamp)

# Increment values and publish data
metricValue += 1
print("Timestamp: " + str(timestamp))
timestamp += 25
print("Timestamp: " + str(timestamp))
publishData(metricValue, timestamp)
